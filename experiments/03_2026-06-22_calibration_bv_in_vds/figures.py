"""Figures for exp 03 (BV-in-VDS) SUMMARY.md.

Reuses exp 03's lightweight figure shapes and adds a per-disease
rejection-reason panel that highlights why the per-disease
sustainability filter was the right move.
"""
...
def fig_acceptance_flow(p1, candidates, draws_used):
    ...
    ax.set_title('Acceptance funnel — exp 03 (BV-in-VDS) (per-disease sustainability filter)')
...
def fig_per_disease_sustain(p1, draws_used):
    """Per-disease sustainability rate at each stage."""
    ...
    # ensemble_summary has *_mean columns aggregated across seeds
    ens_rates = {}
    for d in diseases:
        col = f'sustained_{d}_mean'
        if col in sub.columns:
            # Each draw across 3 seeds: report rate of "passed in all seeds"
            ens_rates[d] = (sub[col] >= 0.999).mean()
        else:
            ens_rates[d] = float('nan')
    ...
    ax.set_ylim(0, 105)
    ax.set_title('Per-disease sustainability — which STI gates the filter?')
...
def fig_rejection_reasons(p1):
    """Of the rejected (non-all-sustained) Phase 1 draws, which disease(s)
    were the cause?"""
    diseases = ['hiv', 'syph', 'ng', 'ct', 'tv']
...
def fig_pass_bands(p1, draws_used):
    ...
    for b in bands:
        col = f'{b}_mean'
        if col in sub.columns:
            ens_hits[b] = float((sub[col] >= 0.5).mean())
        else:
    ...
    ax.barh(y - 0.2, [p1_hits[b]*100 for b in bands], 0.4,
            label='Phase 1 single-seed (n=500)', color='#88a')
...
def main():
    ...
    fig_per_disease_sustain(p1, draws_used)
