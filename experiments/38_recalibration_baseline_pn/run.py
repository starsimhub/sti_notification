...
Re-runs the calibration with three changes relative to exp 35:
  - Baseline partner notification always-on (per-edge-type, per-sex
    stratified rates: stable 20%/80%F/50%M, casual 10%/50%F/25%M).
  - total_pop fixed to 8.7e6 in make_sim so per-agent count outputs
    scale to absolute Zimbabwe-population numbers.
  - 18-dimensional prior space (vs 20 before): added pn.p_notify_stable
    and pn.p_notify_casual as priors; dropped four weak ones from
    exp 34's correlation analysis (rel_trans_latent_half_life,
    m1_conc, f1_conc, fsw_mf_conc_mult).

Two-phase as in exp 35:
  Phase 1: 1500 LHS draws (seed=44, fresh) single seed each.
...
P1_SEED    = int(os.environ.get('P1_SEED', 44))
...
def row_to_sim_pars(row):
    """
    Split a prior row into (sim_pars dict for set_pars_local, pn_pars dict
    for make_sim). pn.* keys are intercepted so the PN rates flow through
    make_sim → make_pn rather than through set_pars_local.
    """
    sim_pars = {}
    pn_notify = {}
