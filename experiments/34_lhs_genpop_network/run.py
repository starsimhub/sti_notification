"""
Exp 34 — LHS over 20 priors with general-population network levers.

Builds on exp 33 (17-dim LHS) by opening three knobs that act directly
on the self-sustaining M↔F general-pop transmission engine identified
in exp 33:
  - syph.rel_trans_latent_half_life ∈ [0.25, 2.0] yr
  - structuredsexual.f1_conc ∈ [0.05, 0.30]
  - structuredsexual.m2_conc ∈ [2.0, 8.0]

If no sustained draw lands in the (relaxed) target band over 20
dimensions, the parameter-only calibration is provably exhausted.

300 LHS draws, single seed each, parallelised on 24 workers. Same 9
targets as exp 32/33. Sustained is enforced as the gate for
ensemble selection (no decay-through-band draws).
"""
import os
...
def main():
    sc.heading(f'Exp 34 — LHS sweep, {N_DRAWS} draws over {len(calib_pars)} priors')
