"""
Exp 39 — Publication-figure data extraction from the exp 38 baseline-PN
ensemble (101 robust draws). Forked from exp 37 with two changes:

  - Source ensemble is exp 38 (baseline PN + total_pop=8.7M).
  - HIV time-series adds `prevalence_15_49` so the UNAIDS-comparable
    denominator is available to plotting.

Per-draw pn_pars are reconstructed from the prior columns so each
sim runs at the calibrated stable/casual notify rates (rather than the
hard-coded defaults in interventions.py).
"""
import os
...
EXP38 = PROJECT_ROOT / 'experiments' / '38_recalibration_baseline_pn'
...
def set_pars_local(sim, pars):
    """Inlined to avoid circular import via run.py."""
...
EXP38_SUMMARY = EXP38 / 'outputs' / 'ensemble_summary.csv'
EXP38_PRIORS  = EXP38 / 'outputs' / 'phase1_priors.csv'
...
def row_to_sim_pars(row):
    """Same split as exp 38 — pn.* keys flow through make_sim → make_pn
    rather than through set_pars_local."""
    sim_pars = {}
    pn_notify = {}
    for col, val in row.items():
        ...
        if isinstance(col, str) and col.startswith('log_'):
            ...
            key = col
            v = float(val)
        if key == 'pn.p_notify_stable':
            pn_notify['stable'] = v
        elif key == 'pn.p_notify_casual':
            pn_notify['casual'] = v
        elif '.' in key:
            sim_pars[key] = v
    pn_pars = {'notify_rates': pn_notify} if pn_notify else None
    return sim_pars, pn_pars
...
def load_ensemble():
    """101 robust draws from exp 38."""
    df = pd.read_csv(EXP38_SUMMARY)
    mask = (df['pass_sustained'] == 1.0) & (df['n_pass_mean'] >= 4)
    ensemble_idxs = df.loc[mask, 'draw_idx'].tolist()
    priors = pd.read_csv(EXP38_PRIORS)
...
def main():
    sc.heading('Exp 39 — pub-figure data extraction from exp 38 ensemble')
    ...
    for _, row in ensemble_priors.iterrows():
        di = int(row['draw_idx'])
        sim_pars, pn_pars = row_to_sim_pars(row.to_dict())
        for s_idx in range(N_SEEDS):
            seed = SEED_BASE + di * 10 + s_idx
            tasks.append({'draw_idx': di, 'seed': seed,
                          'sim_pars': sim_pars, 'pn_pars': pn_pars})
