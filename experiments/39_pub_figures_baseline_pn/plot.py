"""
Publication figures from the 101-draw baseline-PN ensemble (exp 38).

Forked from exp 37/plot.py with one correction:
  - HIV / syph new_infections are now in absolute persons thanks to
    total_pop=8.7e6 in make_sim — multiplied by 1e-3 to display in
    thousands/yr, matching data overlay units.

Note on HIV prevalence panel: UNAIDS `hiv_prevalence` is whole-pop
(denominator ~12M matches Zim 2000 total population), so we overlay it
on model `prevalence` (also whole-pop). The 3-4pp model overshoot is a
known HIV calibration issue (not a denominator artifact). Model 15-49
prevalence is shown as a dashed reference line.
"""
...
TS_Q = HERE / 'outputs' / 'ensemble_ts_quantiles.parquet'
SNAP_Q = HERE / 'outputs' / 'ensemble_snapshots_quantiles.parquet'
FIG_DIR = HERE / 'figures'
FIG_DIR.mkdir(exist_ok=True)

YEAR_LO, YEAR_HI = 1990, 2040
ENSEMBLE_N = 101  # exp 38 robust draws
...
def load_data():
    return pd.read_parquet(TS_Q), pd.read_parquet(SNAP_Q)
...
def fig_syph_timeseries(ts):
    ...
    ax = axes[1, 1]
    # new_infections now in absolute persons (total_pop=8.7M) -> thousands/yr
...
def fig_syph_age_sex_2016(snap):
    year = 2016
    metrics = [
...
def fig_hiv_timeseries(ts):
    hiv_data = pd.read_csv(DATA / 'zimbabwe_hiv_calib.csv')

    fig, axes = plt.subplots(1, 2, figsize=(11, 4.5))

    ax = axes[0]
    # Two denominators on one panel.
    # Whole-pop: model `prevalence` vs UNAIDS `hiv_prevalence` (denominator
    # implied by hiv_n_infected / hiv_prevalence ~12M = total Zim pop).
    # 15-49: model `prevalence_15_49` vs ZIMPHIA point estimates.
...
def main():
    ...
    fig_syph_timeseries(ts)
    fig_syph_definitions(ts)
    fig_syph_age_sex_2016(snap)
    fig_hiv_timeseries(ts)
    fig_sti_timeseries(ts)

    print(f'\nDone. {len(list(FIG_DIR.glob("*.png")))} figures in figures/')
