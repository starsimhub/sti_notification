"""
Exp 40 — Final recalibration with marital coital-decay + client wife
displacement.

Bundles three structural fixes targeting exp 39's calibration misses:
  - New stisim mechanism (feat/marital-act-decay): per-yr linear
    coital decay on stable edges AND client-husband marital-act
    multiplier. Targets the M_client → F_other_via_stable leakage
    route (14% of all syph txn in exp 38; no available knob).
  - HIV recalibration: hiv.beta_m2f upper 0.05 → 0.03; open
    hiv.rel_init_prev in [0.3, 1.5] for the level shift.
  - CT recalibration: ct.beta_m2f range raised [0.02, 0.30] →
    [0.05, 0.50].

Plus drops the two PN priors (max|r| ~0.06 in exp 38). Net prior
space: 19 (was 18 in exp 38; -2 PN + 1 HIV init + 2 new structuredsexual).
Baseline PN stays on at default 0.20/0.10 stable/casual notify rates
(pn_pars=None on every sim — make_sim's wiring falls back to the
defaults baked into interventions.py).

Two-phase as in exp 38:
  Phase 1: 1500 LHS draws (seed=45, fresh) single seed each.
  Phase 2: re-run sustained AND n_pass≥5 candidates with 3 seeds.
"""
import os
...
P1_SEED    = int(os.environ.get('P1_SEED', 45))
...
def row_to_sim_pars(row):
    """Translate a prior row into the sim_pars dict that set_pars_local
    consumes. Exp 40 drops the PN priors, so this is a simple flatten —
    every `module.par` key goes through set_pars_local."""
    sim_pars = {}
