"""
Trajectory selection within exp 17's detectable-mapping NROY.

Differences from exp 13:
- NROY source: exp 17 (post-detectable-fix HM, 0.99% retained).
- Targets: 10 (drops coinfection; remaps syph active to detectable_f/m).
- stisim branch: feat/syph-detectable-state (commit 24bdf58) — must
  be the editable install when this runs.
...
# 10 targets — coinfection pair dropped (coinfection_stats analyzer still
# uses syph.infected, pending a separate stisim patch). Syph active
# remapped to detectable_prevalence_f/m per the WHO Fig 7 mapping.
OBSERVATIONS = {
    ...
    'syph_detectable_f_2016': (0.010, 0.003),
    'syph_detectable_m_2016': (0.006, 0.002),
...
def extract_targets(sim):
    ...
    # Diagnostics (not in likelihood):
    #   syph_prev_f_2016 — full-stage prevalence for invisible-reservoir check.
    #   syph_prev_f_2020_2025 — late-window sustainability (was zero in exp 10 pre-fix).
...
def compute_log_likelihood(row):
    ll = 0.0
    # Widened stds: 2x for non-syph, 3x for syph (matches exp 13).
...
def filter_weight_resample(df):
    ...
    # Use detectable_f extinction as the filter (the observable-side equivalent
    # of exp 13's prev_f filter). Threshold 0.001 because data target is 0.010
    # and the patched model can produce 0 detectable even with non-zero infected.
    n_before = len(df)
    df_alive = df[df['syph_detectable_f_2016'] > 0.001].copy()
    n_after = len(df_alive)
    pct = (n_after / n_before * 100) if n_before else 0
    print(f'Syphilis filter (detectable_f_2016 > 0.001): {n_before} -> {n_after} ({pct:.0f}% survive)')
    ...
    summary = {
        ...
        'invisible_reservoir_ratio_posterior': float(
            (df_posterior['syph_prev_f_2016'] / df_posterior['syph_detectable_f_2016']
...
def main_sim():
    sc.heading(f'Trajectory selection (detectable mapping): '
    ...
    # imap_unordered is fine here: each record carries its own draw_idx tag,
