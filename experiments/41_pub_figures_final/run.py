"""
Exp 41 — Publication-figure data extraction from the exp 40 final
ensemble (200 robust draws). Forked from exp 39 — only difference is
the source ensemble (exp 40 instead of exp 38).

Exp 40 dropped the PN priors (baseline PN runs at fixed default rates
via interventions.py), so this script does not need to reconstruct any
per-draw pn_pars — every draw runs with pn_pars=None.
"""
import os
...
EXP40 = PROJECT_ROOT / 'experiments' / '40_final_recalibration'
...
EXP40_SUMMARY = EXP40 / 'outputs' / 'ensemble_summary.csv'
EXP40_PRIORS  = EXP40 / 'outputs' / 'phase1_priors.csv'
...
def row_to_sim_pars(row):
    """Exp 40 dropped the PN priors, so this is a flat translation —
...
def load_ensemble():
    """200 robust draws from exp 40."""
    df = pd.read_csv(EXP40_SUMMARY)
    mask = (df['pass_sustained'] == 1.0) & (df['n_pass_mean'] >= 4)
    ensemble_idxs = df.loc[mask, 'draw_idx'].tolist()
    priors = pd.read_csv(EXP40_PRIORS)
    ...
    for _, row in ensemble_priors.iterrows():
        ...
        for s_idx in range(N_SEEDS):
            ...
            tasks.append({'draw_idx': di, 'seed': seed,
                          'sim_pars': sim_pars, 'pn_pars': None})
