...
Builds on exp 30 (14-dim LHS) by:
  - Adding `hiv_syph.rel_sus_syph_hiv` and `hiv_syph.rel_trans_syph_hiv`
    as priors (previously fixed at 1.0 in the connector).
  - Adding ZIMPHIA-matched HIV-stratified syph targets (trep+ in HIV+
    and HIV+/HIV- trep ratio).
  - Pulling stage-share + transmission-matrix data from the
    SyphTransmissionEvents analyzer (wired into model.py) instead of
    a run-local monkey-patch.
  - Relaxing the nontrep_f band to [0.01, 0.05] so the HIV-coupling
    lever has room to act.

300 LHS draws, single seed each, parallelised on 24 workers.
"""
import os
...
def main():
    sc.heading(f'Exp 32 — LHS sweep, {N_DRAWS} draws over {len(calib_pars)} priors')
