...
def preset_compare(presets):
    ...
    for metric, bar_label, ts_label in [
        ...
        ("new_infections", "New infections (cumulative)", "New infections"),
    ]:
...
```{python}
# | label: ojs-data
_metrics = ["prevalence", "new_infections", "overtreatment"]
...
# Rename aggregated parquet columns to the schema the OJS explorer expects.
# The Scenario Explorer's TS chart shows median lines only — no ribbon here
# (that lives in the Plotly narrative charts).
_ts = TS.copy()
_ts["disease_label"] = _ts["disease"].map(prep.DISEASE_LABEL)
_ts = _ts.rename(columns={"result_name": "metric", "median": "value"})
...
//| label: ojs-inputs
viewof metric = {
  const opts = [["prevalence", "Prevalence"], ["new_infections", "New infections"],
...
metricLabels = ({prevalence: "End-of-horizon prevalence", new_infections: "New infections (cumulative)", overtreatment: "Overtreatment rate"})
tsLabels = ({prevalence: "Prevalence", new_infections: "New infections"})
...
tsAgg = {
  if (metric !== "prevalence" && metric !== "new_infections") return [];
